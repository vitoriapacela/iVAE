...
from data import SyntheticDataset, CustomSyntheticDataset
...
#     print('Data seed: ', str(args.s))
    ...
    # Training set
#     dset = SyntheticDataset(args.data_path, config.nps, config.ns, config.dl, config.dd, config.nl, args.s, config.p, config.act, uncentered=config.uncentered, noisy=config.noisy, double=factor, one_hot_labels=config.one_hot, simple_mixing=config.simple_mixing, which='train', discrete=config.discrete, identity=config.identity, m_bounds=np.array([-args.m,args.m]), cond_thresh=config.cond_thresh, same_var=config.same_var, norm_A_data=config.norm_A_data)
    train_data = np.load('/projappl/project_2002842/beyond-NonSENS/experiments/real_antti_I.npz')
    dset = CustomSyntheticDataset(X=train_data['x'], U=train_data['u'], S=train_data['s'], A=train_data['A'], m=train_data['m'], L=train_data['L'], device=config.device)
    d_data, d_latent, d_aux = dset.get_dims()

#     loader_params = {'num_workers': 6, 'pin_memory': True} if torch.cuda.is_available() else {}
#     loader_params = {}
#     loader_params = {'num_workers': 0, 'pin_memory': True} if torch.cuda.is_available() else {}
    ...
    # Validation set
#     val_dset = SyntheticDataset(args.data_path, config.nps, config.ns, config.dl, config.dd, config.nl, args.s, config.p, config.act, uncentered=config.uncentered, noisy=config.noisy, double=factor, one_hot_labels=config.one_hot, simple_mixing=config.simple_mixing, which='val', discrete=config.discrete, identity=config.identity, m_bounds=np.array([-args.m,args.m]), cond_thresh=config.cond_thresh, same_var=config.same_var, norm_A_data=config.norm_A_data)
    val_dset = dset
    val_d_data, val_d_latent, val_d_aux = val_dset.get_dims()

    val_data_loader = DataLoader(val_dset, batch_size=config.batch_size, shuffle=config.shuffle, drop_last=True, **loader_params)

    # Test set for plots
#     test_dset = SyntheticDataset(args.data_path, config.nps, config.ns, config.dl, config.dd, config.nl, args.s, config.p, config.act, uncentered=config.uncentered, noisy=config.noisy, double=factor, one_hot_labels=config.one_hot, simple_mixing=config.simple_mixing, which='test', discrete=config.discrete, identity=config.identity, m_bounds=np.array([-args.m,args.m]), cond_thresh=config.cond_thresh, same_var=config.same_var, norm_A_data=config.norm_A_data)
...
#                             model.f.fc[0].weight[i,j] = torch.nn.Parameter(torch.tensor(dset.A_mix[i,j])) + args.init_noise*(torch.rand(1) - 0.5) # noise from init_noise * U(-0.5, 0.5)
            ...
            if args.set_prior:
                # Fix the prior models (mean and variance) to the true prior means and variances

                # set weights to true prior
                with torch.no_grad():
                    model.prior_mean.fc[0].weight = torch.nn.Parameter(torch.tensor(dset.m.T))
                    model.logl.fc[0].weight = torch.nn.Parameter(torch.tensor(dset.l.T))
        ...
        Xt, Ut, St, At = dset.x.to(config.device), dset.u.to(config.device), dset.s.to(config.device), dset.A_mix.to(config.device)

        # load all validation data
        val_X, val_U, val_S, val_A = val_dset.x.to(config.device), val_dset.u.to(config.device), val_dset.s.to(config.device), val_dset.A_mix.to(config.device)
...
#                     perf = mcc(s_true.numpy(), z.cpu().detach().numpy())
                    # why does the above work? should be 
...
#                 perf_all = mcc(dset.s.numpy(), s.cpu().detach().numpy())
                perf_all = mcc(dset.s.cpu().numpy(), s.cpu().detach().numpy())
            except:
                perf_all = 0
...
#                 mix_mcs = mcs_p(model.f.fc[0].weight.data, At, method='cos')
            ...
            if config.wandb:
                ...
                # The following assumes a square mixing matrix
                for i in range(d_data):
                    for j in range(d_data):
    ...
    #           clip_grad_norm(model.parameters(), 2)
        ...
        weights_path_last = ckpt_path + str(config.epochs) + '.pth'
    ...
    return all_perf_hists, loss_hists, perf_hists, val_loss_hists, val_perf_hists, all_perf_mix_hists, all_mcs_mix_hists
