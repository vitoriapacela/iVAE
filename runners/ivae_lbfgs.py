...
#     cond_thresh=config.cond_thresh
    ...
    data_loader = DataLoader(dset, batch_size=bs, shuffle=config.shuffle, drop_last=True, **loader_params)
...
#                 model = DiscreteIVAE(latent_dim=d_latent, data_dim=d_data, aux_dim=d_aux, activation='none', n_layers=config.n_layers, hidden_dim=20, device=config.device)
                model = discrete_simple_IVAE(data_dim=d_data, latent_dim=d_latent, aux_dim=d_aux, hidden_dim=config.hidden_dim, n_layers=config.n_layers, activation=config.activation, slope=.1, interpret=False, simple_g=config.g, device=config.device, simple_prior=config.simple_prior, simple_logv=config.simple_logv, fix_v=config.fix_v, logvar=config.logvar).to(config.device)
            else:
                if config.uncentered:
                    model = u_simple_IVAE(data_dim=d_data, latent_dim=d_latent, aux_dim=d_aux, hidden_dim=config.hidden_dim, n_layers=config.n_layers, activation=config.activation, slope=.1, interpret=False, simple_g=config.g, simple_prior = config.simple_prior, simple_logv=config.simple_logv, noise_level=config.noisy).to(config.device)
                else:
                    model = simple_IVAE(data_dim=d_data, latent_dim=d_latent, aux_dim=d_aux, hidden_dim=config.hidden_dim, n_layers=config.n_layers, activation=config.activation, slope=.1, interpret=False, simple_g=config.g, simple_prior=config.simple_prior, simple_logv=config.simple_logv, noise_level=config.noisy).to(config.device)
...
#                             model.f.fc[0].weight[i,j] = torch.nn.Parameter(torch.tensor(dset.A_mix[i,j])) + args.init_noise*(torch.rand(1) - 0.5) # noise from init_noise * U(-0.5, 0.5)
            ...
            if args.set_prior:
                ...
                with torch.no_grad():
                    #print('m_seg', m_seg.shape)
                    #print('L_var', L_var.shape)
                    #print(dset.m.shape)
...
#                     model.prior_mean.fc[0].weight = torch.nn.Parameter(torch.tensor(m_seg))
#                     model.logl.fc[0].weight = torch.nn.Parameter(torch.tensor(L_var))
        ...
        for epoch in range(1, config.epochs + 1):
            ...
            for i, data in enumerate(data_loader):
                ...
                def closure():
                    ...
                    # set seed for sampling z. the loss becomes deterministic and LBFGS performs better
                ...
                # That is, no hidden layers, n_layers=1 or 2 !!!
                if config.norm_f:
                    # this is not working with LBFGS, but we are keeping the option so we don't forget
                    print("LBFGS WITH MIXING MODEL NORMALIZATION EXPLODES SOON, EVERYTHING GOES TO NAN!!!")
                    with torch.no_grad():
                        h = model.f.fc[0].weight.clone().detach()
#                         print('h', h)
                        model.f.fc[0].weight[:] = torch.nn.Parameter(h.div_(torch.norm(h, dim=0, keepdim=True))) # columns
    
#                 if config.norm_prior:
#                     # try to normalize prior variance
#                     # this may not work with lbfgs
#                     print('NORMALIZING THE PRIOR MODEL DURING LBFGS WILL GIVE NANS!')
#                     with torch.no_grad():
#                         h = model.logl.fc[0].weight.clone().detach()
#                         model.logl.fc[0].weight[:] = torch.nn.Parameter(h.div_(torch.norm(h, dim=0, keepdim=True))) # column-wise
                        
#                         # also normalize prior means
#                         h_ = model.prior_mean.fc[0].weight.clone().detach()
#                         print(h_)
#                         model.prior_mean.fc[0].weight[:] = torch.nn.Parameter(h_.div_(torch.norm(h_, dim=0, keepdim=True))) # column-wise
                        
#                 print('batch loss', loss.item())
                # batch loss
                train_loss += loss.item()
                
                # Ignore this, since we don't have z sampled
...
#                     val_loss, val_z = model.elbo(val_X, val_U) # for DiscreteIVAE
                    val_loss, val_z = model.elbo(val_X, val_U, len(val_dset))
...
#             print('(A, A)', mcs_p(model.f.fc[0].weight.data, At, method='cos'))
#             print('(A, A^T)', mcs_p(model.f.fc[0].weight.data, At.T, method='cos'))
#             print('(A^T, A)', mcs_p(model.f.fc[0].weight.data.T, At, method='cos'))
#             print('(A^T, A^T)', mcs_p(model.f.fc[0].weight.data, At, method='cos'))
