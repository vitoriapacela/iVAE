...
from functions import LBFGS, FullBatchLBFGS, compute_stats, get_grad
...
#                 model.g.fc[0].requires_grad_(False)
        ...
        # Define optimizer
#         optimizer = FullBatchLBFGS(filter(lambda p: p.requires_grad, model.parameters()), lr=1., history_size=10, line_search='Wolfe', debug=True)
...
#         grad, obj = get_grad(optimizer, Xt, St, opfun)
          
#         obj = closure() # would need to define closure earlier, which we don't know if works
#         obj.backward()

        # compute initial objective (and gradient?)
        optimizer.zero_grad()
        loss, _ = model.elbo(Xt, Ut, len(Xt))
        loss.retain_grad()
        loss.backward(retain_graph=True)
        grad = optimizer._gather_flat_grad() # this line is optional
...
#                 loss = optimizer.step(closure)
                ...
                loss, grad, lr, _, _, _, _, _ = optimizer.step(options)
...
#                         print(h)
    #                     model.f.fc[0].weight[:] = torch.nn.Parameter(h.div_(torch.norm(h, dim=1, keepdim=True))) # rows (do not use this line! only for debuggig purposes)
