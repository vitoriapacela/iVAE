...
# def max_perm_mcs_col(A, true_A, method='cos'):
#     """
#     This is wrong as it is. It is taking the permutations and not the combination.
#     Also, it is not clear if we should take the max by row or column.
#     What about just maximizing the sum of the similarities? That would be simpler.
#     MCC or MCS by taking the maximum of all column permutations (without linear assignment).
#     Expects that A and true_A are matrices (2D arrays).
#     """
#     if type(A) != type(true_A):
#         raise ValueError('inputs are of different types: ({}, {})'.format(type(x), type(y)))
#     if isinstance(A, np.ndarray):
#         mean_mcc = np.mean( np.max(get_mcc_np(A, true_A, method), axis=0) )
#     elif isinstance(A, torch.Tensor):
#         mean_mcc = torch.mean( torch.max(get_mcc_pt(A, true_A, method), dim=0).values )
#     else:
#         raise ValueError('not a supported input type: {}'.format(type(x)))
#     return mean_mcc
