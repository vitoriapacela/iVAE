...
# def checkpoint(path, exp_id, seed, iteration, model, optimizer, loss, perf, full_perf, s, val_loss, val_mcc, mix_mcc):
#     sub_path = make_dir(path + str(exp_id) + '/')
#     weights_path = sub_path + str(exp_id) + '_learn-seed_' + str(seed) + '_data-seed_' + str(s) + '_ckpt_' + str(iteration) + '.pth'
...
def checkpoint(path, exp_id, seed, iteration, model, optimizer, loss, perf, full_perf, s, val_loss, val_mcc, mix_mcc, grad, time, verbose=True):
    sub_path = make_dir(path + str(exp_id) + '/')
    weights_path = sub_path + str(exp_id) + '_learn-seed_' + str(seed) + '_data-seed_' + str(s) + '_ckpt_' + str(iteration) + '.pth'
    if verbose:
        print('.. checkpoint at iteration {} ..'.format(iteration))
...
def full_checkpoint(path, exp_id, seed, iteration, model, optimizer, loss, perf, full_perf, s, logpx, logps_cu, logqs_cux, val_loss, val_mcc, mix_mcc, verbose=True):
    sub_path = make_dir(path + str(exp_id) + '/')
    weights_path = sub_path + str(exp_id) + '_learn-seed_' + str(seed) + '_data-seed_' + str(s) + '_ckpt_' + str(iteration) + '.pth'
    if verbose:
        print('.. checkpoint at iteration {} ..'.format(iteration))
